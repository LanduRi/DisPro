...
class Memory(nn.Module):
    ...
    def forward(self, query, keys, train=True):
        ...
        #train
        if train:
            separateness_loss, compactness_loss, J3 = self.gather_loss(query, keys, train)
            updated_query, softmax_score_query, softmax_score_memory = self.read(query, keys)
            updated_memory = self.update(query, keys, train)
            return updated_query, updated_memory, softmax_score_query, softmax_score_memory, separateness_loss, compactness_loss, J3
    ...
    def gather_loss(self, query, keys, train):
        ...
        if train:
            ...
            # disPro loss  ************************************************************
            # query: TxC, mem: MxC, att: TxM
            query = query_reshape
            mem = keys
            att = softmax_score_memory
            Sb = torch.zeros(size=(query.shape[1], query.shape[1])).cuda() # C*C
            Sw = torch.zeros(size=(query.shape[1], query.shape[1])).cuda() # C*C

            N = query.shape[0] # T
            mem_meam = torch.mean(mem, 0)

            _, labels = torch.topk(att, 1, dim=1)
            labels = labels[:, 0] # Tx1
            _, indices = torch.sort(labels)
            query = query[indices, :]
            output, count = torch.unique(labels, sorted=True, return_counts=True)
            
            idx = 0
            for i, j in zip(output, count):
                x = query[idx:idx + j,:]  
                idx += j
                x = x - mem[i]
                xt = torch.t(x)
                j = j.type(torch.FloatTensor)
                Sw += torch.matmul(xt, x) / N

                m = (mem[i].view(-1,query.shape[1]) - mem_meam.view(-1,query.shape[1]))
                mt = torch.t(m)
                Sb += (j/N) * torch.matmul(mt, m)
   
            J3 = torch.trace(Sw)/torch.trace(Sb)
            return gathering_loss, top1_loss, J3
