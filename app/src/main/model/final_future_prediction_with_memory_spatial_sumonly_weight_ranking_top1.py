...
class Decoder_rgbdiff(torch.nn.Module):
    def __init__(self, t_length = 5, n_channel =3):
        super(Decoder_rgbdiff, self).__init__()
        ...
        self.moduleDeconv2 = Basic(256, 256)
...
class convAE(torch.nn.Module):
    def __init__(self, n_channel =3,  t_length = 5, memory_size = 10, feature_dim = 512, key_dim = 512, temp_update = 0.1, temp_gather=0.1):
        ...
        self.decoder_rgbdiff = Decoder_rgbdiff(t_length, 12)
    ...
    def forward(self, x, keys,train=True):
        ...
        if train:
            updated_fea, keys, softmax_score_query, softmax_score_memory, separateness_loss, compactness_loss, J3 = self.memory(fea, keys, train)
            output = self.decoder(updated_fea, skip1, skip2, skip3)
            output_rgbdiff = self.decoder_rgbdiff(updated_fea, skip1, skip2, skip3)

            return output, 2 * output_rgbdiff, fea, updated_fea, keys, softmax_score_query, softmax_score_memory, separateness_loss, compactness_loss, J3
